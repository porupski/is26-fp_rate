...
Five sequential steps. Run from the repo root.
...
Shared helpers live in `scripts/utils_GEE.py` (step 4) and `scripts/utils_results.py` (step 5).
...
# then point 5_'s RESULTS_DIR at the run folder you want to collect:
...
```
1_InterSpeech26_FP_rate/                       ← repo root
├── README.md
├── ENV_SETUP.md
├── requirements.txt
├── scripts/
